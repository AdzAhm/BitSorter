...
- **There are six background tracks and they are one piece of music.** Same key
  (A minor pentatonic), same tempo, same instrument, same four-bar shape; what
  differs is density, ring, register, which way the figure moves on its
  alternate pass, and where the chords go. `MusicTests` pins the scale and the
  sparseness, because a mistyped semitone is invisible to read and obvious to
  hear. A rest is `Rest`, not "negative" -- one track is written an octave down,
  so `semi < 0` would silence it.
