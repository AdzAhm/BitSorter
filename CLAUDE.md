...
- **There are ten background tracks and they are one piece of music.** Same five
  ...
  where the chords go, and which `Voice` plays it. `MusicTests` pins the scale
  and the sparseness, because a mistyped semitone is invisible to read and
  obvious to hear. A rest is `Rest`, not "negative" -- some tracks are written
  an octave down, so `semi < 0` would silence them.

  **Two moods, one scale.** Seven root that collection on A and read as minor;
  ...
  plucked, keys, or the mallets of track 9, the one track that differs in its
  instrument rather than its notes; reverb is one pass over the finished
  buffer, so it costs nothing at runtime.
  ...
  **Nothing under the music, and nothing in it changes value in one sample.**
  ...
  over a millisecond, each bar strikes its own bass note while the last one
  falls away under it, and a note is only dropped once it is below -60 dB.
  `MusicTests` looks above 8 kHz, where no note reaches, and holds hiss and
  clicks to -66 and -54 dBFS.
  ...
  **Mute is one switch for the whole game**, music and cues alike. Every cue has
  ...
  near that Nyquist limit, and it is what keeps ten long uncompressed clips
  affordable -- 27 MiB of heap, against 54 at the cue rate, in a game whose whole
